...
app
  ...
  .post(function (req, res) {
    if (!req.user) {
      ...
      const regExp =
        /^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|playlist\?list=|\&list=)([^#\&\?]*).*/;
      let newPlaylistId = playlistUrl.match(regExp);
      fetch(
        "https://youtube.googleapis.com/youtube/v3/playlistItems?part=contentDetails&maxResults=50&playlistId=" +
          newPlaylistId[2] +
          "&key=" +
          apiKey
      )
...
app.post("/change-password", function (req, res) {
  ...
  } else req.flash("info", "Passwords don't match");
  res.redirect("/dashboard");
});
